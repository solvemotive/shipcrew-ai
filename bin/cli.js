...
const TEAMS = {
  ...
  'ship-crew': 'Full ship — all 16 crew members',
...
function usage() {
  log(`Usage: ${c('bold', 'npx shipcrew <command>')} [options]
...
function buildConfigSection(stack, crewName, agentFiles) {
  ...
  return `## Shipcrew Configuration

> Generated by \`npx shipcrew init\` — crew: **${crewName}**
...
function upsertClaudeMd(cwd, section) {
  ...
  const markerStart = '<!-- shipcrew:start -->';
  const markerEnd = '<!-- shipcrew:end -->';
...
function installCursorRules(cwd, force) {
  const src = path.join(PKG_ROOT, '.cursor', 'rules', 'shipcrew.mdc');
  const destDir = path.join(cwd, '.cursor', 'rules');
  const dest = path.join(destDir, 'shipcrew.mdc');
...
async function cmdInit(args) {
  ...
  if (stack.files.length || fs.existsSync(path.join(cwd, 'package.json'))) {
    ...
    log(`  ${c('green', '✓')} CLAUDE.md ${configStatus} (Shipcrew Configuration)`);
  ...
  log('');
  log(c('green', c('bold', '  ⚓ Shipcrew ready.')));
  log('');
  log(
